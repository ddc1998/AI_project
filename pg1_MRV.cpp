...
void crossing_detect()
{
    ...
    for (int i = 0; i < cnt_varis; i++)
    {
        ...
        {
            if (vari[i].ori == 65 && vari[j].ori != 65)
            { //65:A
                if ((vari[i].coor_x <= vari[j].coor_x) && (vari[i].coor_y >= vari[j].coor_y) && (vari[i].len >= (vari[j].coor_x - vari[i].coor_x + 1)) && (vari[j].len >= (vari[i].coor_y - vari[j].coor_y + 1)))
                {
...
void assign_node()
{
    ...
    state[state_i].value = origin_domain[state[state_i].vari].vals[origin_domain[state[state_i].vari].used];
    origin_domain[state[state_i].vari].used++;
...
int crossing_check()
{
    ...
    for (int i = 0; i < state_i; i++)
    {
        cout << "crossing_check: vari " << state[state_i-1].vari << "with" << state[state_i].vari << endl;
        safe = safe && crossing_check2(state[state_i-1].vari, state[state_i].vari);
        if (safe == 0)
            return 0;
    }
    if (safe)
...
{
    ass.assign[state[state_i].vari] = 0;
    origin_domain[state[state_i].vari].used = 0;
    state[state_i].value = origin_domain[state[state_i].vari].vals[0];
    state_i--;
    if ((origin_domain[state[state_i].vari].used + 1) == origin_domain[state[state_i].vari].cnt_vals)
    {
        return 1000;
    }
    origin_domain[state[state_i].vari].used++;
