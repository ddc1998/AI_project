...
int AC3_or_not(int kk_value,int kk_vari_seq,int ee_vari,int ee_vari_seq){
    for (int m = 0; m < state[0].state_domain[ee_vari].cnt_vals; m++){ //ee的domain m:第幾個ee
        int ee_value = state[0].state_domain[ee_vari].vals[m];
...
void AC3_initial(){
    ...
    for(int i=0; i<cnt_varis;i++){//有幾個variable i:kk的variable
        int save_value[ state[0].state_domain[i].cnt_vals ]={0};
        int save_cnt=0;
        for(int k=0;k<state[0].state_domain[i].cnt_vals;k++){//kk的domain  k:第幾個kk
            int kk_value = state[0].state_domain[i].vals[k];       
            safe=1;    
            for(int j=0; j<cro_vari[i].crossing_cnt; j++){//kk會對到幾個位置的ee  j:第幾個位置
                ...
                safe = safe&&AC3_or_not(kk_value, kk_vari_seq, ee_vari, ee_vari_seq);
            }
            if(safe==1){
        ...
        copy(save_value, save_value + save_cnt, state[0].state_domain[i].vals);
        state[0].state_domain[i].cnt_vals = save_cnt;
    }
}
...
int main()
{
    ...
    while (getline(fin, str)){
        ...
        setup_root();
        for(int i=0;i<cnt_varis;i++){
            cout<<"vari="<<i<<" domain_size="<<state[0].state_domain[i].cnt_vals<<endl;
        }
        cout<<endl;
        AC3_initial();
        for(int i=0;i<cnt_varis;i++){
